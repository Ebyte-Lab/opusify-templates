...
export const PortraitsSection: React.FC<PortraitsSectionProps> = ({ onImageSelect }) => {
  const [visibleCount, setVisibleCount] = useState<number>(6);

  const portraitImages = galleryImages.filter((img) => img.category === 'portraits');
  const displayedImages = portraitImages.slice(0, visibleCount);
  ...
  return (
    <section id="portraits" className="p-4 md:p-8 lg:p-12 border-b border-secondary">
